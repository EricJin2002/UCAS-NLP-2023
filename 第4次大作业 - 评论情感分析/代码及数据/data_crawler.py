...
# GET https://bgm.tv/subject/404804/comments
...
def html_add_to_comment(html_txt, comment_dict):
    ...
    comment_box = soup.find_all('div', id='comment_box')
    if len(comment_box) > 0:
        comments = comment_box[0].find_all('div', class_='text')
        if len(comments) > 0:
            for comment in comments:
                ...
                for span in spans:
                    ...
                    for class_name in class_names:
                        if class_name.startswith("stars") and class_name[-1].isdigit():
                            score = class_name[-1]
                            if score == "0":
                                score = "10"
                            # print("score:", score)
                p = comment.find_all('p')
                ...
                if score is not None:
                    comment_dict[score] = comment_dict.get(score, []) + [words]
            return True
        else:
            return False
    else:
